...
func runDay02() {
     ...
     */
    var totalSquareFeet: Int = 0
    
    for (length, width, height) in presents {
        let sideA = length * width
        let sideB = width * height
        let sideC = length * height
        let totalSideArea = 2 * (sideA + sideB + sideC) + min(sideA, sideB, sideC)
        totalSquareFeet += totalSideArea
    }
    print("Part one solution: \(totalSquareFeet)")
     ...
     */
    var totalRibbonFeet: Int = 0
    for (length, width, height) in presents {
        let sidePerimeter = 2 * (length + width + height - max(length, width, height))
        totalRibbonFeet += sidePerimeter + length * width * height
    }
    print("Part two solution: \(totalRibbonFeet)")
}
