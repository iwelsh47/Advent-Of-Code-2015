...
struct AuntSue {
    let number: Int
    let information: [String: Int]
    
    init(from data: String) {
        let bits = data.components(separatedBy: .whitespaces)
        number = Int(bits[1].dropLast())!
        var idx = 2
        var info: [String: Int] = [:]
        while idx < bits.count {
            let key = String(bits[idx].dropLast())
            let value = Int(bits[idx + 1].replacingOccurrences(of: ",", with: ""))!
    ...
    func exactlyMatches(_ data: [String: Int]) -> Bool {
        for (name, count) in data {
            if information.keys.contains(name) && information[name] != count {
                return false
            }
        }
        return true
    ...
    func rangedMatches(_ data: [String: Int]) -> Bool {
        for (name, count) in data {
            if ["cats", "trees"].contains(name) {
                if information.keys.contains(name) && information[name]! <= count {
                    return false
                }
            } else if ["pomeranians", "goldfish"].contains(name) {
                if information.keys.contains(name) && information[name]! >= count {
                    return false
                }
            } else {
...
func runDay16() {
    let input = loadData(from: "Day16").components(separatedBy: .newlines)
     ...
     */
    let auntSues = input.map{ AuntSue(from: $0) }
    let processedInfo = ["children": 3, "cats": 7, "samoyeds": 2, "pomeranians": 3, "akitas": 0, "vizslas": 0,
                         "goldfish": 5, "trees": 3, "cars": 2, "perfumes": 1]
    print("Part one solution: \(auntSues.filter{ $0.exactlyMatches(processedInfo) }.map{ $0.number })")
