...
class OneMillionLights {
    ...
    init(rows: Int, columns: Int, state: Int) {
        self.brightness = Array(repeating: state, count: rows * columns)
        self.state = Array(repeating: state > 0, count: rows * columns)
        self.rows = rows
        self.columns = columns
    }
    
    func turnOn(from: (row: Int, col: Int), to: (row: Int, col: Int)) {
        for r in from.row...to.row {
            for c in from.col...to.col {
                state[r * columns + c] = true
                brightness[r * columns + c] += 1
            }
        }
    }
    
    func turnOff(from: (row: Int, col: Int), to: (row: Int, col: Int)) {
        for r in from.row...to.row {
            for c in from.col...to.col {
                state[r * columns + c] = false
                if brightness[r * columns + c] > 0 {
                    brightness[r * columns + c] -= 1
                }
            }
        }
    ...
    func toggle(from: (row: Int, col: Int), to: (row: Int, col: Int)) {
        for r in from.row...to.row {
            for c in from.col...to.col {
                state[r * columns + c].toggle()
                brightness[r * columns + c] += 2
            }
        }
    }
    
    var countOn: Int { state.reduce(0, { $0 + ($1 ? 1 : 0) }) }
    var totalBrightness: Int { brightness.reduce(0, { $0 + $1 }) }
}
...
func runDay06() {
    ...
    for instruction in input.split(separator: "\n") {
        ...
        if instruction.hasPrefix("turn on") {
            ...
            let (endRow, endCol) = bits[4].split(separator: ",").map{ Int($0)! }.splat()
            lights.turnOn(from: (startRow, startCol), to: (endRow, endCol))
        }
        if instruction.hasPrefix("turn off") {
            let (startRow, startCol) = bits[2].split(separator: ",").map{ Int($0)! }.splat()
            let (endRow, endCol) = bits[4].split(separator: ",").map{ Int($0)! }.splat()
            lights.turnOff(from: (startRow, startCol), to: (endRow, endCol))
        }
        if instruction.hasPrefix("toggle") {
            let (startRow, startCol) = bits[1].split(separator: ",").map{ Int($0)! }.splat()
            let (endRow, endCol) = bits[3].split(separator: ",").map{ Int($0)! }.splat()
            lights.toggle(from: (startRow, startCol), to: (endRow, endCol))
        }
    }
    print("Part one solution: \(lights.countOn)")
     ...
     */
    print("Part two solution: \(lights.totalBrightness)")
}
