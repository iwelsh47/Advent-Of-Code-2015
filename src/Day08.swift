...
//  AoC2015
//
//  Created by Ivan Welsh on 27/12/2024.
//
...
func unescape(_ string: String) -> String {
    ...
    while idx < string.endIndex {
        ...
        if string[idx] == "\\" && nextIdx < string.endIndex && string[nextIdx] == "x"  {
            let hexStart = string.index(after: nextIdx)
            let hexEnd = string.index(hexStart, offsetBy: 2)
            result.append(String(UnicodeScalar(Int(string[hexStart..<hexEnd], radix: 16)!)!))
            idx = hexEnd
        } else if string[idx] == "\\" && nextIdx < string.endIndex && (string[nextIdx] == "\\" || string[nextIdx] == "\"") {
            result.append(string[nextIdx])
...
func reescape(_ string: String) -> String {
    var result = "\""
    var idx = string.startIndex
    while idx < string.endIndex {
        let nextIdx = string.index(after: idx)
...
func runDay08() {
    let input = loadData(from: "Day08").components(separatedBy: .newlines)
    ...
    var total = 0
    for string in input {
        let stringMemory = unescape("\(string.dropFirst().dropLast())")
        total += string.count - stringMemory.count
    }
    ...
    total = 0
    for string in input {
        let stringEncoded = reescape(string)
        total += stringEncoded.count - string.count
    }
